    ...
    public function process(){
        ...
        {                       
            ...
            if($result){
                ...
                if($this->login_model->get_user_profile($result->id) != false){
                    $sess_data['user_profile'] = $this->login_model->get_user_profile($result->noktp);
                }
                $this->session->set_userdata($sess_data);
                /*update last logon user*/
                $this->db->query("UPDATE qrcode_user SET last_logon=now() WHERE username='".$result->username."' AND password='".$result->password."'");

                echo json_encode(array('status' => 200, 'message' => 'Login berhasil', 'logged' => TRUE, 'token' => md5($result->noktp), 'user' => $sess_data['user'], 'user_profile' => isset($sess_data['user_profile'])?$sess_data['user_profile']:[] ));
            }else{
